...
class DatabaseManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.db.run(
        `INSERT INTO game_sessions 
         (id, session_name, session_code, admin_password, max_players, status) 
         VALUES (?, ?, ?, ?, ?, 'waiting')`,
  ...
  async updatePlayerCoins(playerId, newCoins) {
    return new Promise((resolve, reject) => {
      this.db.run(
        'UPDATE players SET coins = ?, last_activity = CURRENT_TIMESTAMP WHERE id = ?',
        [newCoins, playerId],
        function(err) {
          if (err) reject(err);
          else resolve(this.changes > 0);
  ...
  async addFlagSolution(playerId, flagId) {
    return new Promise((resolve, reject) => {
      this.db.run(
        'INSERT OR IGNORE INTO player_solutions (player_id, flag_id, solved_at) VALUES (?, ?, ?)',
        [playerId, flagId, new Date().toISOString()],
        function(err) {
          if (err) reject(err);
  ...
  async removeFlagSolution(playerId, flagId) {
    return new Promise((resolve, reject) => {
      this.db.run(
        'DELETE FROM player_solutions WHERE player_id = ? AND flag_id = ?',
        [playerId, flagId],
        function(err) {
          if (err) reject(err);
    ...
    return new Promise((resolve, reject) => {
      ...
      db.serialize(() => {
        ...
        db.run(`DELETE FROM player_solutions 
                     ...
                     [sessionId, sessionId]);
        db.run('DELETE FROM game_events WHERE session_id = ?', [sessionId]);
        db.run('DELETE FROM attacks WHERE session_id = ?', [sessionId]);
        db.run('DELETE FROM flags WHERE session_id = ?', [sessionId]);
